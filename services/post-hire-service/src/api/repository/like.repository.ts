...
export interface ILikeRepository {
	...
	getAllLikeByListPost(
		postIds: string[]
	): Promise<Record<string, FeelingResponse[]>>;
...
export class likeRepository implements ILikeRepository {
	...
	constructor(likeCollection: DatabaseAdapter) {
	...
	public likeComment = async (
		...
		return await this.likeCollection.like.insertOne({
			authorId: userId,
			commentId: commentId,
	...
	public unlikeComment = async (
		...
		const result = await this.likeCollection.like.deleteOne({
			commentId: commentId,
			authorId: userId,
			type: FeelingType.COMMENT,
		});
	...
	public getAllLikeByListComment = async (
		commentIds: string[]
	): Promise<Record<string, FeelingResponse[]>> => {
		const allLikes = await this.likeCollection.like
			.find<FeelingResponse>({
	...
	public likePost = async (
		...
		const existingLike = await this.likeCollection.like.findOne({
			authorId: userId,
			postId: postId,
		});
		if (existingLike) {
			throw new APIError({ message: "Like already exists" });
		}
		return await this.likeCollection.like.insertOne({
			authorId: userId,
			postId: postId,
	...
	public getAllLikePost = async (
		...
		return await this.likeCollection.like
			.find<FeelingResponse>({
				postId: postId.trim(),
				type: FeelingType.POST.trim(),
			})
			.sort({ createdAt: -1 })
			.toArray();
	};

	public getAllLikeByListPost = async (
		postIds: string[]
	): Promise<Record<string, FeelingResponse[]>> => {
		const allLikes = await this.likeCollection.like
			.find<FeelingResponse>({
