...
class RabbitMQManager {
	...
	async connect(): Promise<void> {
		...
		try {
			...
			this.connection = await client.connect(config.rabbitmq.uri);
	...
	private async setupExchanges(): Promise<void> {
		...
		await channel.assertExchange(this.RETRY_EXCHANGE, "direct", {
			durable: true,
		});
	...
	async getChannel(queueName: string): Promise<Channel> {
		...
		try {
			...
			console.error(
				`❌ Failed to create channel for "${queueName}":`,
				error,
			);
			throw error;
		}
	...
	async sendToQueue(queue: string, message: any): Promise<void> {
		try {
			...
			if (!sent) {
				console.warn(
					`⚠️ Message buffer is full for "${queue}", waiting...`,
				);
	...
	async consume(
		...
		try {
			...
			await channel.consume(
				...
				async (msg: ConsumeMessage | null) => {
					...
					try {
						...
						console.log(
							`📨 Received message from "${queue}" (retry: ${retryCount}/${maxRetries})`,
						);
						await handler(content);
						channel.ack(msg);
						console.log(
							`✅ Message processed successfully from "${queue}"`,
						);
					} catch (error) {
						console.error(
						...
						if (retryCount < maxRetries) {
							...
							channel.ack(msg);
							console.log(
								`🔄 Message sent to retry queue (attempt ${retryCount + 1}/${maxRetries})`,
							);
						} else {
							channel.nack(msg, false, false);
							console.log(
								`💀 Message moved to DLX after ${maxRetries} retries`,
							);
						}
					}
				},
			...
			console.log(
				`👂 Listening to queue "${queue}" (max retries: ${maxRetries})`,
			);
		} catch (error) {
			console.error(
				`❌ Failed to consume messages from "${queue}":`,
				error,
			);
			throw error;
		}
	...
	private async retryMessage(
		...
		channel.publish(this.RETRY_EXCHANGE, queue, msg.content, {
			persistent: true,
			headers: {
	...
	async consumeDLX(
		...
		await channel.consume(dlxQueue, async (msg: ConsumeMessage | null) => {
			if (!msg) return;
			...
			try {
				...
				const retryCount =
					msg.properties.headers?.["x-retry-count"] || 0;
				logger.info(
					`💀 Processing DLX message from "${queue}" (failed after ${retryCount} retries)`,
				);
				await handler(content);
				channel.ack(msg);
			} catch (error) {
				logger.error(`❌ Error processing DLX message:`, error);
				channel.nack(msg, false, false);
			}
		});
	}
	...
	async assertQueue(
		...
		const { durable = true, enableDLX = true } = options;
		try {
			const channel = await this.getChannel(queue);

			// Setup main queue
			const queueOptions: any = { durable };

			// Enable DLX if requested
			if (enableDLX) {
				queueOptions.deadLetterExchange = this.DLX_EXCHANGE;
				queueOptions.deadLetterRoutingKey = queue;

				// Also create the DLX queue
				await channel.assertQueue(queue + this.DLX_QUEUE_SUFFIX, {
					durable,
				});
				await channel.bindQueue(
					queue + this.DLX_QUEUE_SUFFIX,
					this.DLX_EXCHANGE,
					queue,
				);
			}
			...
			await channel.bindQueue(queue, this.RETRY_EXCHANGE, queue);

			logger.info(
				`✅ Queue "${queue}" asserted successfully (DLX: ${enableDLX})`,
			);
		} catch (error) {
